...
def calculate_projectile_range(angle, velocity, gravity=9.81):
    ...
    Returns:
    - float: The range of the projectile in meters.
    """
    angle_rad = np.radians(angle)
    return (velocity**2 * np.sin(2 * angle_rad)) / gravity

def simulate_projectile_motion(
    initial_velocity=20, angle_range=(10, 80), num_angles=10, gravity=9.81
):
    """
    Simulate projectile motion and plot the range as a function of launch angle.

    Parameters:
    - initial_velocity (float): Initial velocity in m/s. Default is 20.
    - angle_range (tuple): Tuple of (min_angle, max_angle) in degrees. Default is (10, 80).
    - num_angles (int): Number of angles to simulate. Default is 10.
    - gravity (float): Gravitational acceleration in m/s^2. Default is 9.81.
    """
    angles = np.linspace(angle_range[0], angle_range[1], num_angles)
    ranges = [calculate_projectile_range(angle, initial_velocity, gravity) for angle in angles]

    plt.figure(figsize=(10, 6))
    plt.plot(angles, ranges, marker='o', color='b', label='Projectile Range')
    plt.xlabel("Launch Angle (degrees)", fontsize=12)
    plt.ylabel("Range (meters)", fontsize=12)
    plt.title(
        f"Projectile Range vs. Launch Angle\nInitial Velocity = {initial_velocity} m/s",
        fontsize=14,
    )
